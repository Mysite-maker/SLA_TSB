...
<body>
  ...
  <div id="tab-content-kanban" class="tab-content active">
    ...
    <!-- Side Drawer Backdrop & Content -->
    <div class="drawer-backdrop" id="report-drawer-backdrop">
      <div class="drawer-content">
        <div class="drawer-header">
          <div class="panel-title" style="margin-bottom: 0; border-bottom: none; padding-bottom: 0;">
            <span id="drawer-title-icon">✍️</span> <span id="drawer-title-text">บันทึกรายงานประจำวัน</span>
          </div>
      ...
      <section class="board-container">
        ...
        <div class="control-row flex flex-col md:flex-row md:justify-between md:items-center gap-3">
          <div class="date-selector-group flex flex-row items-center gap-2 w-full md:w-auto">
            ...
            <button type="button" class="btn-preset" id="btn-open-drawer" style="background: linear-gradient(135deg, var(--accent-blue), #2563eb); border: none; color: white; padding: 0.45rem 1rem; border-radius: 6px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 0.35rem; box-shadow: var(--glow-blue); transition: all 0.3s ease;">
              <span>✍️</span> บันทึกรายงานใหม่
            </button>
          </div>
  ...
  <script>
    ...
    // Set default summary date range (Monday of current week to today)
    ...
    summaryStartDateInput.value = getBangkokMonday();
    ...
    function renderSummaryTable() {
      ...
      summaryReports.forEach(report => {
        ...
        // Extract date from created_at
         // จัด Format วันที่รายงานใหม่จาก YYYY-MM-DD เป็น DD-MM-YYYY
        let displayDate = '';
        if (report.created_at) {
          const datePart = report.created_at.split(' ')[0]; // แยกเอาเฉพาะ YYYY-MM-DD
          const parts = datePart.split('-'); // หั่นเป็นชิ้น [YYYY, MM, DD]
          if (parts.length === 3) {
            displayDate = parts[2] + '-' + parts[1] + '-' + parts[0]; // ประกอบกลับเป็น DD-MM-YYYY
          }
        }
        ...
        tr.innerHTML = \`
          ...
          <td>
  <div class="sec-text" style="font-size: 0.85rem; color: \${report.status === 'Done' ? '#10b981' : 'var(--text-secondary)'};">\${escapeHtml(report.work_details)}</div>
</td>
    ...
    // Drawer controller logic
    const drawerBackdrop = document.getElementById('report-drawer-backdrop');
    ...
    function openDrawer(mode = 'create') {
      if (mode === 'edit') {
        drawerTitleText.textContent = 'แก้ไขรายงานปฏิบัติงาน';
        drawerTitleIcon.textContent = '💾';
      } else {
        drawerTitleText.textContent = 'บันทึกรายงานประจำวัน';
        drawerTitleIcon.textContent = '✍️';
      }
    ...
    // Drawer buttons/backdrop listeners
    ...
    // Summary Date Range change listeners
    document.getElementById('summary-start-date').addEventListener('change', () => {
      document.querySelectorAll('.btn-preset').forEach(btn => btn.classList.remove('active'));
      fetchSummaryReports();
    });

    document.getElementById('summary-end-date').addEventListener('change', () => {
      document.querySelectorAll('.btn-preset').forEach(btn => btn.classList.remove('active'));
      fetchSummaryReports();
    });
